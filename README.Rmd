...
```{r gs_cite_data, echo=T,eval=F}

nca <- get_gs_cite_data(id=="DpaI7XMAAAAJ") # not run, data pre-loaded for example

```
```{r gs_str,echo=T,eval=T}
str(nca)
```

## `get_pct_first()`

This is a function that calculates the share of a given scholar's citations that are from first-authored publications.
